    ...
    return (
      <div className={styles.page}>
        <div className={styles.pageHeader}>
          <div>
            <Skeleton h={32} w={340} />
            <div style={{ marginTop: 8 }}><Skeleton h={16} w={260} /></div>
          </div>
        </div>
        <div className={styles.analysisLayout}>
          <aside className={styles.locationList}>
            {[1,2,3,4].map(i => (
              <div key={i} style={{ padding: '1rem', borderBottom: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 8 }}>
                <Skeleton h={16} w="70%" />
                <Skeleton h={12} w="50%" />
                <Skeleton h={8} w="90%" r={4} />
              </div>
            ))}
          </aside>
          <div className={styles.analysisMain}>
            <div className={styles.riskGaugeCard} style={{ display: 'flex', gap: '2rem', alignItems: 'center' }}>
              <Skeleton w={120} h={120} r={60} />
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
                <Skeleton h={20} w="40%" />
                <Skeleton h={14} w="60%" />
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginTop: 8 }}>
                  {[1,2,3,4,5,6].map(i => <Skeleton key={i} h={56} r={8} />)}
                </div>
              </div>
            </div>
  ...
  if (!loading && results.length === 0) {
    return (
      <div className={styles.page}>
        <div className={styles.pageHeader}>
          <div>
            <h1 className={styles.pageTitle}><span className={styles.aiIcon}>🤖</span> AI Su Kalitesi Analizi</h1>
            <p className={styles.pageSubtitle}>Gemini destekli anomali tespiti, risk skorlama ve doğal dil sorgulama</p>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '5rem 2rem', gap: '1.5rem', color: 'var(--text-muted)', textAlign: 'center' }}>
          <div style={{ fontSize: '4rem' }}>🌊</div>
          <h2 style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--text-primary)' }}>Henüz Analiz Verisi Yok</h2>
          <p style={{ maxWidth: 360, lineHeight: 1.6 }}>AI analiz verileri yüklenemedi. Lütfen sayfayı yenileyin.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
          {[
            ...
            <div key={s.label} className="card" style={{ padding: '1.25rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
              ...
              <div>
                <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600 }}>{s.label}</p>
        ...
        <div className={styles.analysisLayout}>
          ...
          <aside className={styles.locationList}>
            ...
            {results.map((r, i) => (
              ...
              >
                ...
                {/* Risk bar */}
                <div style={{ height: 4, background: 'var(--bg-primary)', borderRadius: 4, marginTop: 6, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${r.result.riskScore}%`, background: r.result.riskColor, borderRadius: 4, transition: 'width 0.4s ease' }} />
                </div>
                {/* Mini param bars */}
          ...
          <div className={styles.analysisMain}>
            ...
            <div className={styles.riskGaugeCard}>
              <div className={styles.gaugeLeft}>
                ...
                <div className={styles.gaugeInfo}>
                  ...
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 4 }}>📅 {cur.sample.date}</span>
                </div>
              </div>
            ...
            {cur.result.anomalies.length > 0 ? (
              ...
              <div className={styles.summaryCard} style={{ borderColor: 'rgba(0,255,136,0.2)', background: 'rgba(0,255,136,0.04)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <span style={{ fontSize: '1.5rem' }}>✅</span>
                  <div>
                    <p style={{ fontWeight: 600, color: '#00ff88' }}>Anomali Tespit Edilmedi</p>
                    <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginTop: 2 }}>Tüm parametreler normal sınırlar içinde.</p>
                  </div>
                </div>
              </div>
            )}
          ...
          <aside className={styles.chatSidebar}>
            ...
            {results.map((r, i) => (
              <div key={i} className={styles.sidebarItem}
                onClick={() => { setSelected(i); setTab('analysis'); }}
            ...
            <div style={{ marginTop: '1.5rem', padding: '1rem', background: 'rgba(110,142,251,0.08)', borderRadius: 'var(--radius-md)', border: '1px solid rgba(110,142,251,0.2)' }}>
              <p style={{ fontSize: '0.72rem', color: '#6e8efb', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Genel Durum</p>
              <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                {criticalCount > 0 ? `🚨 ${criticalCount} kritik nokta acil müdahale bekliyor.` : '✅ Kritik seviyede alan yok.'}
              </p>
              <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginTop: 4 }}>
                Ortalama risk skoru: <strong style={{ color: 'var(--text-primary)' }}>{avgScore}/100</strong>
              </p>
            </div>
          </aside>
        </div>
