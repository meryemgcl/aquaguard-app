...
export async function POST(request: NextRequest) {
  try {
    ...
    // Hoşgeldin maili gönder (arka planda, hatayı ignore et)
    sendWelcomeMail(email, name, userRole).catch(console.error);

    const response = NextResponse.json({
      success: true,
      message: 'Kayıt başarılı! Hoşgeldin e-postası gönderildi.',
      token,
      user,
      emailSent: true,
    });
